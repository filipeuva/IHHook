...
namespace Hooks_Ui {
    ...
    static bool Seh(const char* tag, void* ctx, F&& fn) {
        ...
        __except (EXCEPTION_EXECUTE_HANDLER) { spdlog::warn("[SEH] {} ctx={}", tag, ctx); return false; }
    #else
    ...
    static void* PickAnchorModelForLayout(void* layout) {
        ...
        uint32_t bestCount = 0;
        {
            ...
            for (auto& kv : g_layoutByModel) {
                ...
                if (cnt > bestCount) { bestCount = cnt; bestModel = kv.first; }
            }
        }
        return bestModel;
    }
    ...
    static void DumpCommonPortsForLayout(void* layout) {
        ...
        if (!model) { spdlog::info("[PORT] layout={} no model", layout); return; }
        ...
        Seh("DumpCommonPorts", model, [&]{
            ...
            for (uint32_t i = 0; i < count; ++i) {
                ...
                Seh("ReadCommonSid", node, [&]{ sid = *reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(node) + 0x6C); });
    ...
    static bool TryResolveAnchorForLayout(void* layout, std::pair<void*, const void*>& out)
    {
        ...
        if (!port) {
            spdlog::info("[ROOT] layout={} model={} has no default common port and none enumerated; skip.", layout, model);
            return false;
        }
    ...
    static void PollHotkeys() {
        ...
        if (JustPressed(VK_F9)) {
            ...
            std::vector<void*> layouts;
            { std::lock_guard<std::mutex> _l(g_mx); for (auto& L : g_knownLayouts) layouts.push_back(L); }
            for (void* L : layouts) DumpCommonPortsForLayout(L);
        }
        if (JustPressed(VK_F10)) {
            spdlog::info("[HK] F10 -> resolve anchors (IsHaveModelNodeCommon + GetModelNodeCommonInternal/Generic)");
    ...
    // -----------------------------------------------------------------------------//
    // text hooks
    void __fastcall SetTextForModelNodeTextHook(void* uix, void* nodeText, void* textUnit, const char* rawText, bool isLocalized) {
    ...
    void CreateHooks() {
        ...
        spdlog::info("[UI-HOOK] Hooks installed (ROOT via Model::GetModelNodeCommonInternal/IsHaveModelNodeCommon).");
    }
